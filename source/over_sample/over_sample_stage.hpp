...
namespace zldsp::oversample {
        ...
        void downsample(std::span<FloatType *> buffer, const size_t num_samples) {
            ...
            for (size_t chan = 0; chan < buffer.size(); ++chan) {
                ...
                for (size_t i = 0; i < num_samples; ++i) {
                    ...
                    for (size_t k = 0; k < symmetric_size; ++k) {
                        output += (shifted_delay_line[k] + shifted_delay_line[symmetric_shift - k]) * down_coeff_[k];
                    }
                    chan_data[i] = output;
    ...
    private:
        std::vector<FloatType> up_coeff_{};
        FloatType up_coeff_center_{FloatType(0)};
        size_t up_coeff_center_pos_{0};
        std::vector<std::vector<FloatType> > up_delay_lines_{};

        std::vector<FloatType> down_coeff_{};
        FloatType down_coeff_center_{FloatType(0)};
        std::vector<std::vector<FloatType> > down_delay_lines_{};
        size_t down_center_pos_{0};
        std::vector<std::vector<FloatType> > down_center_delay_lines_{};
