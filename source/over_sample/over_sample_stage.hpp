            ...
            for (size_t chan = 0; chan < buffer.size(); ++chan) {
                ...
                for (size_t i = 0; i < num_samples; ++i) {
                    ...
                    if (UseSIMD) {
                        ...
                        for (size_t k = 0; k < delay_line.size() / 2; ++k) {
                            output += (delay_line[k] + delay_line[delay_line.size() - k - 1]) * up_coeff_[k];
                        }
            ...
            for (size_t chan = 0; chan < buffer.size(); ++chan) {
                ...
                for (size_t i = 0; i < num_samples; ++i) {
                    ...
                    if (UseSIMD) {
                        ...
                        for (size_t k = 0; k < delay_line.size() / 2; ++k) {
                            output += (delay_line[k] + delay_line[delay_line.size() - k - 1]) * down_coeff_[k];
                        }
                    }
                    chan_data[i] = output;
                    std::rotate(delay_line.begin(), delay_line.begin() + 1, delay_line.end());
